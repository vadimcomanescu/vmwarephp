...
class PropertyCollector extends \Vmwarephp\ManagedObject {
	...
	private function findRequestedObjectInCollectionResult($collectionResult, $managedObjectType) {
		...
		foreach ($collectionResult as $managedObject)if (strpos(get_class($managedObject), $managedObjectType) !== false || $managedObject->reference->type === $managedObjectType) return $managedObject;
